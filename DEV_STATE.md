...
**Updated:** 23 September 2026. Internal record; not public showcase content.

The active Windows workspace is `LOCAL_HOME\Documents\sqe-platform`.
Use [SQE Developer Hub](SQE_DEVELOPER_HUB.html) for current navigation and [Workspace Guide](WORKSPACE-GUIDE.md) for source and evidence locations.

The Hub's [Repository Maps section](SQE_DEVELOPER_HUB.html#repository-maps) centralises CodeGraph, Understand Anything and Graphify links, generation records and refresh instructions. All three were generated on 23 September 2026. Use the linked validation records to check freshness. Maps are saved snapshots, not application acceptance.

The Hub also links the historical Code Atlas and the separately identified [Archify diagram skill](https://github.com/tt-a1i/archify). Skill setup does not establish a new SQE diagram or application verification.

## Current Position

| Workstream | Evidence-Based Position | Next Work |
|---|---|---|
| Consolidation | Initial import complete: 29 Mac files and two approved instructions passed identity checks | Reconcile remaining dirty work and held imports |
| Python workbench | Engagement, evidence review, MATE, relationship review and release functionality exist; September 19 startup and focused HTTP viewing checks passed | Current complete browser/write/release journey remains unverified |
| Next.js pilot | Original source preserved; identical across the compared Windows/Mac trees | Establish authorised Convex access if the live pilot is tested |
| Public showcase | Separate Zauner Site published; version 3 includes item-specific RFI histories and dated sources | Operational SQE integration is not implemented by this showcase |
| Swift iOS client | Source imported; native evidence retained; current Mac state not checked in this update | Vortex to investigate alternatives and compare iPhone-first delivery; Copy cause remains unresolved |

## Current Review Direction

The owner has reopened mobile alternatives for investigation. Swift, React Native/Expo, mobile web/PWA, wrappers and credible alternatives may be compared. No rewrite or migration is approved. Options to finish the existing Python workbench or develop its newer interface remain under review.

Use the [complete Vortex brief](LOCAL_HOME/sqe-private/handoffs/2026-09-23-vortex-full-review-brief.md) and [code/evidence index](docs/ace/CODE-AND-EVIDENCE-INDEX.md). The brief has been prepared, not sent to Vortex. No independent verdict has been received.

Zauner is the approved sample engagement and its supplied material is authorised for the public showcase. The operational application's existing controls have not been changed by that publication.

An earlier Codex explanation understated August implementation. Source review confirms existing capture-to-review integration tests, release lifecycle services, client routes and Swift networking. Historical suite totals are not new passes on this dirty tree.

Windows branch: `codex/sqe-workspace-consolidation`.
HEAD and preserved `main`: `478a7d8cd9c97d0bc43bd4d41ea460d589d4003a`.
The imported files and current documentation changes remain uncommitted.

Mac delivery workspace: `LOCAL_HOME/Developer/sqe-platform-release-layout`.
Its candidate was verified unchanged at `40bae2640b23eb05496a093d09a4c20186403879` during Stage 3.
The Windows and Mac working directories are not automatically synchronised.

## Current Evidence And Limits

The [Stage 3 report](LOCAL_HOME/sqe-private/consolidation-20260918/stage3/STAGE-3-EXECUTION-REPORT.md) records recovery, hashes and repository checks.
Its SHA-256 is `3b1cef8a3a780369dc524818645ec950bf844414deb84411eb43e065fd09e27e`.
The documentation reconciliation record lives in `LOCAL_HOME\sqe-private\consolidation-20260918\stage3-docs`.

No new app build, runtime test, deployment or iOS acceptance follows from these consolidation checks.
The full iOS programme, physical checks, signing, private service and historical ancestry decision remain open.
The original Copy finding is unresolved. A passing retry does not establish its cause or correction.

Mac Python approval/G0 rehydration changes remain held as a separate behaviour decision.
CI/provider changes, LibreOffice changes and other dirty variants remain preserved rather than silently integrated.
Historical issue statuses below were not refreshed from Linear in this update.

The previous Swift-only recommendation is reopened for investigation. Existing code and approved diagnostic evidence remain preserved. Codemagic, paid distribution and operational Production deployment remain outside this review authority.

The recorded signing profile expired on 21 September; renewal status is unknown. Current Mac HEAD, physical-device access and private-service readiness remain unverified here. D2's final execution result was not inspected in the September 23 review.

The public Zauner Site has a separate source commit, `8a06ae2756fa7034091937828667d3fd0c6210c1`. Native publication succeeded on September 23. Twenty-one focused content/rendering checks and a fresh review passed; browser layout/accessibility remain unverified. See the [showcase record](LOCAL_HOME/sqe-private/zauner-example-20260919/correspondence/CONVERSATION-LAYOUT-20260923.md).

## Historical Record

The original development-state text follows unchanged in meaning, including its dated claims and references.
Its earlier “current”, “verified” and “complete” statements apply to those historical revisions, not this working tree.
Exact original bytes are retained in the private documentation backup and the Stage 3 recovery archive.
