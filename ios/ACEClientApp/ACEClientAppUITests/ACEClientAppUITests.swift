...
final class ACEClientAppUITests: XCTestCase {
    ...
    private func waitForCoverageState(in app: XCUIApplication, scenario: String) -> Bool {
        ...
        case "loading": expected = app.activityIndicators["Loading current release"]
    ...
    private func coverageActionLabels(for scenario: String) -> [String] {
        ...
        case "emptyRelease", "emptyEngagement", "unavailable", "unexpected", "connection", "timeout", "invalidResponse":
            return ["Refresh current release", "Sign out"]
        case "keychainRead":
    ...
    private func setNativeAccessibilityToggle(_ label: String, path: [String], enabled: Bool, in settings: XCUIApplication) -> Bool {
        ...
        while true {
            ...
            case .succeed:
                ...
                return true
            case .fail:
                let failure = current == nil ? "unreadable" : "exhausted"
                addScreenshot(of: settings, named: "Accessibility settings \(label) \(failure)")
                XCTFail("Settings control \(label): \(failure), expected \(enabled), current \(String(describing: current)), taps \(taps)")
                return false
            case .tap:
            ...
            predicate: NSPredicate { _, _ in
                app.staticTexts[expected].exists
                    ...
                    || app.progressIndicators[expected].exists
                        || app.activityIndicators[expected].exists
            },
            object: nil
        )
    ...
    func testAllControlledScenariosShowExpectedStateAndAudit() throws {
        let states = [
            ("loading", "Loading current release"), ("emptyRelease", "No current release is available."), ("emptyEngagement", "No current release is available."),
