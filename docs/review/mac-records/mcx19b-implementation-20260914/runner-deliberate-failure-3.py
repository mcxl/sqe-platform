...
backup=base/'runner-deliberate-original-test-3.swift'
assert not backup.exists()
backup.write_bytes(original)
record={'question':'Does the updated selected-test runner retain the deliberately incorrect POST expectation and actual GET result?','selector':'ACEClientAppTests/ACEClientAppTests/testRequestIsGETAndHasNoCache','expected':'POST','actualExpected':'GET','sourceOriginalSha256':hashlib.sha256(original).hexdigest(),'sourceProbeSha256':hashlib.sha256(changed).hexdigest(),'started':time.time()}
source.write_bytes(changed)
code=99
try:
    env=dict(os.environ,PYTHONPYCACHEPREFIX=str(base/'python-cache'))
    with (base/'runner-deliberate-failure-3.log').open('w') as out:
    ...
    (base/'runner-deliberate-failure-3.json').write_text(json.dumps(record,indent=2))
