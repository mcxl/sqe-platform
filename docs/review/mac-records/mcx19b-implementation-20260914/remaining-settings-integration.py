...
e=base/'remaining-settings-integration'; e.mkdir(mode=0o700,exist_ok=False)
...
record={'question':'Can native controls enable and restore Reduce Motion, Increase Contrast and landscape one setting at a time, with actual app observations?','started':time.time(),'candidate':m.candidate(allow_dirty=True),'buildInputs':inputs,'templateSha256':m.sha256(template),'productSha256':m.directory_hash(template.parent),'buildEvidence':str(base/'signin-prompt-build-2.xcresult'),'acceptedCoverageCases':0}
...
try:
    ...
    record['steps']=[]
    baseline={'boldText':False,'reduceMotion':False,'increaseContrast':False,'orientation':'portrait'}
    for label,key,value in [('reduce-motion','reduceMotion',True),('increase-contrast','increaseContrast',True),('landscape','orientation','landscape')]:
        target={**baseline,key:value}
        observed=m.native_flags(identifier,template,e,label,{'mode':'set',**target})
        record['steps'].append({'label':label,'requested':target,'observed':observed})
        print(json.dumps(record['steps'][-1]),flush=True)
        restored=m.native_flags(identifier,template,e,label+'-restore',{'mode':'set',**baseline})
        record['steps'].append({'label':label+'-restore','requested':baseline,'observed':restored})
        print(json.dumps(record['steps'][-1]),flush=True)
