...
args=["/Library/Frameworks/Python.framework/Versions/3.14/bin/python3",str(root/"tools/ace_ios_local.py"),"--evidence-root",str(ev/"functional-runs"),"selected","--device","iPhone 17 Pro Max"]
for test in selectors:args.extend(["--test",test])
start=datetime.datetime.now(datetime.timezone.utc).isoformat()
r=subprocess.run(args,cwd=root,capture_output=True,text=True)
(ev/"native-functional-console.log").write_text(r.stdout+r.stderr)
...
(ev/"native-functional-command.json").write_text(json.dumps(record,indent=2))
