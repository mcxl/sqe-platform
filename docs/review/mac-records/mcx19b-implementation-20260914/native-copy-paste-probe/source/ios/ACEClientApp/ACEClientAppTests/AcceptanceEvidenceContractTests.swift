...
extension AcceptanceEvidenceContractTests {
    ...
    private var approvedSanitizedBaseManifest: [String: String] {
        ...
        ["docs/specs/2026-08-24-ace-ios-read-only-client-application.md": "bce02156da2e55749347678feb206173d2c0ff07bb0c40ba3d88f21d1aca2a7b"]
    }
...
extension AcceptanceEvidenceContractTests {
    ...
    func testCopyControlsUITest() throws {
        ...
        XCTAssertEqual(valueRowSource.components(separatedBy: "private var copyButton").count - 1, 1, "ValueRow must contain one dedicated copy-control implementation")
        XCTAssertEqual(valueRowSource.components(separatedBy: "ClipboardWriteContract.write(").count - 1, 1, "ValueRow must have one protected clipboard action")
        XCTAssertTrue(valueRowSource.contains("Label(\"Copy\""), "The copy control must have a visible Copy label")
        XCTAssertTrue(valueRowSource.contains(".accessibilityLabel(\"Copy \\(field.label)\")"), "The copy button must retain its accessible name")
        ...
        XCTAssertTrue(valueRowSource.contains("if let confirmation"), "ValueRow must show the copy confirmation")
        XCTAssertTrue(valueRowSource.contains("Text(confirmation)"), "Copy confirmation must remain visible to accessibility services")
...
extension AcceptanceEvidenceContractTests {
    ...
    func testFixtureInspection() throws {
        ...
        let approvedLiterals: Set<String> = [
            ...
            "Fictional owner", "Fictional summary", "FICTIONAL-REF-001", "Loading", "Loading current release",
            "No actions are available.", "No conclusion is available.", "No current release is available.",
            "OPEN", "RELEASED", "Reset saved sign-in", "Saved sign-in could not be read. Try again.",
