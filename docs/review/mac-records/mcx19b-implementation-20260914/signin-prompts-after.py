...
e=base/'signin-prompts-after'; e.mkdir(mode=0o700,exist_ok=False)
template=base/'derived-diagnostic/Build/Products/ACEClientAppUITests_iphonesimulator26.4-x86_64.xctestrun'
inputs=json.loads((base/'signin-prompt-build-inputs.json').read_text())
...
record={'question':'Do both appearances retain readable sign-in prompts, secure password entry and a full native audit after the demonstrated colour correction?','started':time.time(),'candidate':m.candidate(allow_dirty=True),'buildInputs':inputs,'templateSha256':m.sha256(template),'productSha256':m.directory_hash(template.parent),'buildEvidence':str(base/'signin-prompt-build.xcresult'),'acceptedCoverageCases':0}
