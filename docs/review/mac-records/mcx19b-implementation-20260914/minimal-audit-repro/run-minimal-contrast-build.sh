...
build() {
  local result=0
  date -u +%FT%TZ > "$results_dir/minimal-audit-repro-contrast-build.started"
  caffeinate -i xcodebuild build-for-testing \
    ...
    -resultBundlePath "$results_dir/minimal-audit-repro-contrast-build.xcresult" \
    ...
    > "$results_dir/minimal-audit-repro-contrast-build.log" 2>&1
  result=$?
  printf '%s\n' "$result" > "$results_dir/minimal-audit-repro-contrast-build.exit"
  date -u +%FT%TZ > "$results_dir/minimal-audit-repro-contrast-build.finished"
  tail -n 12 "$results_dir/minimal-audit-repro-contrast-build.log"
  return "$result"
}
...
case "$mode" in
  build)
    ...
    ;;
  all)
    run_selected_test minimal-audit-repro-all testMinimalAuditReproAllAudits
    ;;
  contrast)
