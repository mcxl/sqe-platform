...
#if DEBUG
struct MinimalAuditReproView: View {
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 20) {
                SectionHeader("Current Release")
                ...
                ReleaseCard {
                    Text("Release Details")
                        .font(.headline)
                        .accessibilityAddTraits(.isHeader)
                    ValueRow(field: .engagementName, value: "Fictional Engagement")
                    ValueRow(field: .reviewStatus, value: "RELEASED")
                    ValueRow(field: .releaseVersion, value: "1")
                    ValueRow(field: .publishedAt, value: "2026-08-24T10:15:30Z")
                }
                SectionHeader("Actions")
                ReleaseCard {
                    Text("Action 1")
                        .font(.headline)
                        .accessibilityAddTraits(.isHeader)
                    ValueRow(field: .actionDescription, value: "Fictional action")
                    ValueRow(field: .actionOwner, value: "Fictional owner")
                    ValueRow(field: .actionTargetDate, value: "2026-08-25")
                    ValueRow(field: .actionStatus, value: "OPEN")
                }
                Text("Minimal audit reproduction bottom")
            }
            .frame(maxWidth: .infinity, alignment: .leading)
        }
    }
