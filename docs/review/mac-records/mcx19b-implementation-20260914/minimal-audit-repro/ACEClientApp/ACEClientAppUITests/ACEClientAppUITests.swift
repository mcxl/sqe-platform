...
final class ACEClientAppUITests: XCTestCase {
    ...
    func testMinimalAuditReproContrastOnlyAfterScroll() throws {
        ...
        assertMinimumActionTargets(in: app)
        addScreenshot(of: app, named: "Minimal audit reproduction — post-scroll — \(requiredAppearance())")
        try assertAccessibilityAudit(in: app, scenario: "minimal-audit-repro-post-scroll", auditType: .contrast)
    }

    func testMinimalAuditReproPixelStableAllAudits() throws {
        let app = launch("minimalAuditRepro")
        defer { app.terminate() }
        let bottom = app.staticTexts["Minimal audit reproduction bottom"]
        XCTAssertTrue(bottom.waitForExistence(timeout: 5), "Minimal audit reproduction must exist")
        ...
        var stableSince = observationStart
        var previous: Data?
        var firstShot: XCUIScreenshot?
        var lastShot: XCUIScreenshot?
        var stable = false
        var captureCount = 0
        while ProcessInfo.processInfo.systemUptime - observationStart < 10 {
            let shot = scrollView.screenshot()
            let pixels = shot.pngRepresentation
            captureCount += 1
            if firstShot == nil { firstShot = shot }
            let now = ProcessInfo.processInfo.systemUptime
            if previous != pixels { stableSince = now }
            lastShot = shot
            if previous == pixels && now - stableSince >= 0.5 { stable = true; break }
            previous = pixels
        }
        guard stable, let firstShot, let before = lastShot else {
            XCTFail("Scroll-view pixels did not remain identical during the observation window")
            return
        }
        for (name, shot) in [("Pixel observation first", firstShot), ("Pixel observation stable before audit", before)] {
            let attachment = XCTAttachment(screenshot: shot)
            attachment.name = name
            attachment.lifetime = .keepAlways
            add(attachment)
        }
        print("ACE_PIXEL_STABILITY before=true captures=\(captureCount) elapsed=\(ProcessInfo.processInfo.systemUptime - observationStart)")
        defer {
            let after = scrollView.screenshot()
            let attachment = XCTAttachment(screenshot: after)
            attachment.name = "Pixel observation after full audit"
            attachment.lifetime = .keepAlways
            add(attachment)
            print("ACE_PIXEL_STABILITY beforeAfterIdentical=\(before.pngRepresentation == after.pngRepresentation)")
        }
        try assertAccessibilityAudit(in: app, scenario: "minimal-audit-repro-pixel-stable-post-scroll")
    }
