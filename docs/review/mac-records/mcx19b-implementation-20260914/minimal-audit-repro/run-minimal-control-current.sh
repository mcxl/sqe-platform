...
build() {
  local result=0
  date -u +%FT%TZ > "$results_dir/minimal-audit-repro-build.started"
  caffeinate -i xcodebuild build-for-testing \
    ...
    -resultBundlePath "$results_dir/minimal-audit-repro-build.xcresult" \
    ...
    > "$results_dir/minimal-audit-repro-build.log" 2>&1
  result=$?
  printf '%s\n' "$result" > "$results_dir/minimal-audit-repro-build.exit"
  date -u +%FT%TZ > "$results_dir/minimal-audit-repro-build.finished"
  tail -n 12 "$results_dir/minimal-audit-repro-build.log"
  return "$result"
}
...
case "$mode" in
  build)
    ...
    ;;
  all)
    run_selected_test minimal-audit-repro-control-current testMinimalAuditReproAllAudits
    ;;
  contrast)
