...
def stable_environment(value):
 identity=json.loads(json.dumps(runner.environment_identity(value)))
 identity['runtime'].pop('lastUsage',None)
 return identity
assert stable_environment(environment)==stable_environment(manifest['environment']), 'Stable environment changed'
(base/'settings-read-environments.json').write_text(json.dumps({'before':manifest['environment'],'after':environment,'excludedField':'runtime.lastUsage only; observed difference is test execution timestamp, not tools or runtime identity.'},indent=2)+chr(10))
