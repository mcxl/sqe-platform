...
e=base/'no-actions-current-build';e.mkdir(mode=0o700,exist_ok=False)
...
record={'question':'Does the original noActions post-scroll unrestricted audit failure remain on the current unchanged iOS source?', 'started':time.time(),'sourceHashes':sourceHashes,'reviewCommit':'b9ba714d99862e9ef5572b7eb2a1b1d6471d0932','buildManifest':str(manifestPath),'executionHelper':str(helper),'executionHelperSha256':m.sha256(helper),'acceptedCoverageCases':0}
try:
 ...
 selector='ACEClientAppUITests/ACEClientAppUITests/testClippingNoActionsStandaloneAudit'
