...
final class ACEClientAppUITests: XCTestCase {
    ...
    func testNativeCopyPasteProbe() {
        continueAfterFailure = false
        let app = launch("release")
        let copy = app.buttons["Copy Engagement name"]
        XCTAssertTrue(copy.waitForExistence(timeout: 5))
        copy.tap()
        XCTAssertTrue(app.staticTexts["Copied Engagement name."].waitForExistence(timeout: 5))
        addScreenshot(of: app, named: "Copy actual engagement value")
        app.terminate()
        let destination = launch("signIn")
        let field = destination.textFields["Username"]
        XCTAssertTrue(field.waitForExistence(timeout: 5))
        field.tap()
        field.press(forDuration: 1)
        addScreenshot(of: destination, named: "Native Paste menu")
        let paste = destination.descendants(matching: .any).matching(NSPredicate(format: "label == %@", "Paste")).firstMatch
        XCTAssertTrue(paste.waitForExistence(timeout: 5), "Native Paste command must be available")
        paste.tap()
        XCTAssertEqual(field.value as? String, "Fictional Engagement", "Native Paste must reproduce the value copied by the release control")
        addScreenshot(of: destination, named: "Actual clipboard value pasted")
