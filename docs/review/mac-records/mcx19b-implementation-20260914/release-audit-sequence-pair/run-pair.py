...
e=base/'release-audit-sequence-pair'
...
try:
 ...
 for name,skip in [('control','0'),('omit-initial','1')]:
  run=e/name;run.mkdir()
  runfile=run/'selected.xctestrun';m.configure_xctestrun(Path(build['template']),runfile,{'ACE_UI_TEST_APPEARANCE':'light','ACE_DIAGNOSTIC_SKIP_INITIAL_AUDIT':skip})
  ...
  assert 'ACE_DIAGNOSTIC_INITIAL_AUDIT skipped='+('true' if skip=='1' else 'false') in process['stdout']
...
raise SystemExit(0 if len(record['runs'])==2 and not record.get('error') and record['originalCandidateUnchanged'] and not record['restorationErrors'] else 1)
