...
for name in CommandLine.arguments.dropFirst() {
    guard let source=CGImageSourceCreateWithURL(URL(fileURLWithPath:name) as CFURL,nil),let image=CGImageSourceCreateImageAtIndex(source,0,nil),let cs=CGColorSpace(name:CGColorSpace.sRGB) else {fatalError("Cannot read PNG")}
