...
def command(command: list[str], cwd: Path | None = None, timeout: int = 1800, env: dict[str, str] | None = None) -> dict[str, object]:
    ...
    try:
        ...
        return {"command": command, "start": started, "end": datetime.now(timezone.utc).isoformat(),
                "exit": None, "error": str(error), "seconds": round(time.monotonic() - started_clock, 3)}
    return {"command": command, "exit": result.returncode, "stdout": result.stdout, "stderr": result.stderr,
            "start": started, "end": datetime.now(timezone.utc).isoformat(), "seconds": round(time.monotonic() - started_clock, 3)}
...
def restore_simctl_settings(identifier: str, previous: dict[str, str], evidence: Path, device: str) -> list[str]:
    failures: list[str] = []
    ...
    (evidence / "restoration" / f"{device}-simctl-commands.json").parent.mkdir(parents=True, exist_ok=True)
    (evidence / "restoration" / f"{device}-simctl-commands.json").write_text(json.dumps(records, indent=2, sort_keys=True), encoding="utf-8")
    return failures
...
def environment_identity(environment: object) -> dict[str, object]:
    ...
    return {key: environment.get(key) for key in ("python", "xcode", "runtime", "devices")}
...
def run_coverage(cases: tuple[Case, ...], evidence: Path, retained_pilot: dict[str, object] | None = None,
    ...
    try:
        for device, data in devices.items():
            ...
            initial_boot_states[device] = simulator_state(identifier)
            if initial_boot_states[device] == "Shutdown":
                # Restore even if boot succeeds but bootstatus later fails.
                booted_by_runner.add(device)
            boot_required_simulator(identifier, initial_boot_states[device])
        ...
        for index, (device, settings, batch_cases) in enumerate(coverage_batches(execution_cases), 1):
            if candidate() != candidate_record:
                raise RunnerError("candidate identity changed before native batch")
            data = devices[device]
            assert isinstance(data, dict)
            native_flags(str(data["udid"]), Path(str(build_record["template"])), evidence, f"{index:03d}-set", {
            ...
            records.append(run_batch(device, str(data["udid"]), settings, batch_cases, build_record, evidence, index))
        ...
        for device in initial_boot_states:
            ...
            if before is not None:
                restoration_failures.extend(f"{device}:{field}" for field in restore_simctl_settings(identifier, before, evidence, device))
                try:
                    observed_simctl = current_simctl_settings(identifier)
                    (evidence / "restoration" / f"{device}-simctl-readback.json").write_text(
                        json.dumps({"expected": before, "observed": observed_simctl}, indent=2, sort_keys=True), encoding="utf-8")
                    if observed_simctl != before:
                        restoration_failures.append(f"{device}:simctl-readback")
                except RunnerError:
                    restoration_failures.append(f"{device}:simctl-readback")
            if device in previous_native_flags:
                try:
                    restored = native_flags(identifier, Path(str(build_record["template"])), evidence, f"{device}-restore", {"mode": "set", **previous_native_flags[device]})
                    if restored != previous_native_flags[device]:
                        restoration_failures.append(f"{device}:native-flags")
                except RunnerError:
                    restoration_failures.append(f"{device}:native-restore")
            if device in booted_by_runner:
                try:
...
def validate_full_gate(pilot_manifest: Path, review_ledger: Path, candidate_record: dict[str, object]) -> dict[str, object]:
    ...
    if resources.get("remainingRawEvidenceBytes") is None or resources.get("requiredFreeBytes") is None:
        raise RunnerError("pilot resource projection is incomplete")
    raw_maximums = [directory_bytes(Path(str(batch["resultBundle"])).parent) for batch in batches]
    image_maximums = [len(attachment_name_map(Path(str(batch["attachmentDirectory"])))) for batch in batches]
    if not raw_maximums or not image_maximums:
        raise RunnerError("pilot evidence cannot measure raw and image maxima")
    max_raw = max(raw_maximums)
    max_images = max(image_maximums)
    if resources.get("pilotMaxRawCaseBytes") != max_raw or resources.get("pilotMaxImageCount") != max_images:
        raise RunnerError("pilot resource record does not match retained evidence maxima")
    if resources.get("remainingImageCount") != (836 - len(eligible_pilot_cases(pilot))) * max_images:
        raise RunnerError("pilot image projection does not match retained evidence maxima")
    projected = (836 - len(eligible_pilot_cases(pilot))) * max_raw
    ...
    if int(scratch) < minimum_scratch:
        raise RunnerError("pilot scratch projection is below retained build products")
    minimum_required = (2 * int(projected)) + int(scratch) + MIN_FREE_BYTES
    if int(resources["remainingRawEvidenceBytes"]) != projected or int(scratch) < 0 or int(resources["requiredFreeBytes"]) < minimum_required:
        raise RunnerError("pilot resource projection is invalid")
    if shutil.disk_usage(ROOT).free < int(resources["requiredFreeBytes"]):
        raise RunnerError("Mac free space no longer meets the approved full-run projection")
    review_measurement = ledger.get("reviewMeasurement")
    expected_images: set[str] = set()
    for batch in batches:
        attachments = Path(str(batch.get("attachmentDirectory", "")))
        expected_images.update(sha256(path) for path in attachments.rglob("*") if path.is_file() and path.suffix.lower() in {".png", ".jpg", ".jpeg"})
    if (not isinstance(review_measurement, dict) or type(review_measurement.get("elapsedSeconds")) is not int
            or review_measurement["elapsedSeconds"] <= 0 or not isinstance(review_measurement.get("inspectedImageSha256"), list)
            or set(review_measurement["inspectedImageSha256"]) != expected_images):
        raise RunnerError("pilot image review measurement is incomplete")
    if not expected_images:
        raise RunnerError("pilot has no retained images for review")
    review_seconds = resources.get("perImageReviewSeconds")
    measured_per_image = (review_measurement["elapsedSeconds"] + len(expected_images) - 1) // len(expected_images)
    if (not isinstance(review_seconds, int) or review_seconds <= 0 or resources.get("projectedImageInspectionSeconds")
            != int(resources["remainingImageCount"]) * review_seconds or review_seconds != measured_per_image
            or int(resources["projectedImageInspectionSeconds"]) > 8 * 60 * 60):
        raise RunnerError("pilot image inspection exceeds the eight-hour limit")
    ...
    if not isinstance(reviewed, list) or set(reviewed) != expected_images or not expected_images:
