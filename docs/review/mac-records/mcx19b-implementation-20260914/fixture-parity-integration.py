...
e=base/'fixture-parity-integration'; e.mkdir(mode=0o700,exist_ok=False)
template=base/'derived-diagnostic/Build/Products/ACEClientAppUITests_iphonesimulator26.4-x86_64.xctestrun'
inputs=json.loads((base/'fixture-parity-build-2-inputs.json').read_text())
...
record={'question':'Do all eleven affected mandatory fixture cases expose their production screen text and controls at default light settings, including secure sign-in recovery? This is a focused control/layout check, not coverage acceptance.','started':time.time(),'candidate':m.candidate(allow_dirty=True),'buildInputs':inputs,'templateSha256':m.sha256(template),'productSha256':m.directory_hash(template.parent),'buildEvidence':str(base/'fixture-parity-build-2.xcresult'),'acceptedCoverageCases':0}
