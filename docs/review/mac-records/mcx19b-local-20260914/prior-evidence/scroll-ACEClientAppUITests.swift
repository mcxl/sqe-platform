...
final class ACEClientAppUITests: XCTestCase {
    ...
    func testClippingNoActionsStandaloneAudit() throws {
        try runOriginalScenarioAudit(
            scenario: "noActions",
            expected: "No actions are available."
        )
    }

    func testClippingNoConclusionStandaloneAudit() throws {
        try runOriginalScenarioAudit(
            scenario: "noConclusion",
            expected: "No conclusion is available."
        )
    }

    func testAccessibilityAuditFirstFiveOriginalScenarioOrder() throws {
        let states = [
            ("loading", "Loading"),
            ("emptyRelease", "No current release is available."),
            ("emptyEngagement", "No current release is available."),
            ("noConclusion", "No conclusion is available."),
            ("noActions", "No actions are available.")
        ]
        for (scenario, expected) in states {
            try runOriginalScenarioAudit(scenario: scenario, expected: expected)
        }
    }
    ...
    private func runOriginalScenarioAudit(scenario: String, expected: String) throws {
        ...
        XCTAssertEqual(XCTWaiter.wait(for: [expectedState], timeout: 5), .completed, "Scenario \(scenario)")
        assertMinimumActionTargets(in: app)
        if scenario == "noConclusion" || scenario == "noActions" {
            scrollUntilVisible(app.staticTexts[expected], in: app, field: expected)
        }
        addScreenshot(of: app, named: "Controlled state — \(scenario) — \(requiredAppearance())")
        try assertAccessibilityAudit(in: app, scenario: scenario)
    ...
    func testClippingSettingsAndRetentionProbe() {
        guard let expected = clippingExpectedDeviceSettings() else {
            return
        }
        print("ACE_A11Y_AUDIT_TYPE_SYMBOLS \(Self.accessibilityAuditTypeSymbolsJSON())")
        let app = launchClippingDiagnostic("release")
        defer { app.terminate() }

        addScreenshot(
            of: app,
            named: "Clipping settings probe — \(expected.appearance) — \(expected.contentSize)",
            snapshot: XCUIScreen.main.screenshot()
        )
        guard verifyClippingPortrait(in: app) else { return }
        _ = verifyClippingDeviceSettings(in: app, expected: expected)
    }

    func testClippingReleaseBeforeCopy() throws {
        try runClippingAudit(
            appScenario: "release",
            container: "release",
            state: "before-copy",
            scenario: "clipping-release-before-copy",
            tapCopy: false
        )
    }

    func testClippingReleaseAfterCopy() throws {
        try runClippingAudit(
            appScenario: "release",
            container: "release",
            state: "after-copy",
            scenario: "clipping-release-after-copy",
            tapCopy: true
        )
    }

    func testClippingStandaloneBeforeCopy() throws {
        try runClippingAudit(
            appScenario: "copyConfirmation",
            container: "standalone",
            state: "before-copy",
            scenario: "clipping-standalone-before-copy",
            tapCopy: false
        )
    }

    func testClippingStandaloneAfterCopy() throws {
        try runClippingAudit(
            appScenario: "copyConfirmation",
            container: "standalone",
            state: "after-copy",
            scenario: "clipping-standalone-after-copy",
            tapCopy: true
        )
    }

    private func clippingExpectedDeviceSettings() -> (appearance: String, contentSize: String)? {
        ...
        guard ProcessInfo.processInfo.environment["ACE_UI_TEST_APPEARANCE"] == nil else {
            XCTFail("Clipping diagnostics must not receive a forced appearance")
            return nil
        }
        guard ProcessInfo.processInfo.environment["ACE_EXPECTED_EFFECTIVE_INTERFACE_STYLE"] == expectedAppearance,
              ProcessInfo.processInfo.environment["ACE_EXPECTED_CONTENT_SIZE_CATEGORY"] == expectedContentSize else {
            XCTFail("Clipping diagnostics require supported normal-device settings")
            return nil
        }
        return (expectedAppearance, expectedContentSize)
    }

    private func launchClippingDiagnostic(_ scenario: String) -> XCUIApplication {
        XCUIDevice.shared.orientation = .portrait
        let app = launchWithNormalDeviceSettings(scenario)
        return app
    }

    private func verifyClippingPortrait(in app: XCUIApplication) -> Bool {
        let portrait = XCTNSPredicateExpectation(
            predicate: NSPredicate { _, _ in app.frame.height > app.frame.width },
            object: nil
        )
        guard XCTWaiter.wait(for: [portrait], timeout: 5) == .completed else {
            XCTFail("The clipping diagnostic must run in portrait orientation")
            return false
        }
        return true
    }

    private func verifyClippingDeviceSettings(
        in app: XCUIApplication,
        expected: (appearance: String, contentSize: String)
    ) -> Bool {
        let indicator = app.staticTexts["Effective interface style"]
        guard indicator.waitForExistence(timeout: 5) else {
            XCTFail("Appearance indicator must exist")
            return false
        }
        let displayedAppearance = XCTNSPredicateExpectation(
            predicate: NSPredicate(format: "label == %@", expected.appearance),
            object: indicator
        )
        let displayedContentSize = XCTNSPredicateExpectation(
            predicate: NSPredicate(format: "value == %@", expected.contentSize),
            object: indicator
        )
        let settingsSettled = XCTWaiter.wait(
            for: [displayedAppearance, displayedContentSize],
            timeout: 5
        ) == .completed
        let actualAppearance = indicator.label
        let actualContentSize = (indicator.value as? String) ?? "unavailable"
        print(
            "ACE_MCX19_CLIPPING_SETTINGS expectedAppearance=\(expected.appearance) expectedContentSize=\(expected.contentSize) actualAppearance=\(actualAppearance) actualContentSize=\(actualContentSize) settled=\(settingsSettled)"
        )
        XCTAssertEqual(actualAppearance, expected.appearance, "The displayed view must use the simulator's expected appearance")
        XCTAssertEqual(actualContentSize, expected.contentSize, "The displayed view must use the simulator's expected content size")
        return actualAppearance == expected.appearance && actualContentSize == expected.contentSize
    }

    private func runClippingAudit(
        appScenario: String,
        container: String,
        state: String,
        scenario: String,
        ...
        let app = launchClippingDiagnostic(appScenario)
        let screenshotName = "Clipping diagnostic — \(container) — \(state) — \(expected.appearance) — \(expected.contentSize)"
        var capturedDiagnosticScreenshot = false
        defer {
            if !capturedDiagnosticScreenshot {
                addScreenshot(of: app, named: screenshotName, snapshot: XCUIScreen.main.screenshot())
            }
            app.terminate()
        }

        guard verifyClippingPortrait(in: app) else { return }
        guard verifyClippingDeviceSettings(in: app, expected: expected) else { return }

        let engagementName = app.descendants(matching: .any)
            .matching(NSPredicate(format: "label == %@", "Engagement name: Fictional Engagement"))
            .firstMatch
        let engagementNameExists = engagementName.waitForExistence(timeout: 5)
        guard engagementNameExists, engagementName.isHittable else {
            XCTFail("Engagement name must remain visible for the clipping diagnostic")
            return
        }

        if tapCopy {
            let copyButton = app.buttons["Copy Engagement name"]
            guard copyButton.waitForExistence(timeout: 5), copyButton.isHittable else {
                XCTFail("Copy Engagement name must be hittable")
                return
            }
            copyButton.tap()
            let confirmation = app.staticTexts["Copied Engagement name."]
            guard confirmation.waitForExistence(timeout: 5) else {
                XCTFail("Copy confirmation must be available to VoiceOver")
                return
            }
            assertClippingConfirmationVisible(confirmation, in: app)
            guard engagementName.isHittable else {
                XCTFail("Engagement name must remain visible after copying")
                return
            }
        }

        assertMinimumActionTargets(in: app)
        logClippingEngagementElements(in: app, combinedEngagementName: engagementName, scenario: scenario)
        addScreenshot(
            of: app,
            named: screenshotName,
            snapshot: XCUIScreen.main.screenshot()
        )
        capturedDiagnosticScreenshot = true
        try assertAccessibilityAudit(in: app, scenario: scenario)
    }

    private func assertClippingConfirmationVisible(_ confirmation: XCUIElement, in app: XCUIApplication) {
        XCTAssertTrue(confirmation.exists, "Copy confirmation must exist before visibility verification")
        scrollUntilVisible(confirmation, in: app, field: "Copy confirmation")
        let viewport = app.scrollViews.firstMatch
        XCTAssertTrue(confirmation.isHittable, "Copy confirmation must be hittable after scrolling")
        XCTAssertTrue(
            isFullyVisible(confirmation, in: viewport),
            "Copy confirmation must intersect the scroll viewport after scrolling"
        )
    }

    private func logClippingEngagementElements(
        in app: XCUIApplication,
        combinedEngagementName: XCUIElement,
        scenario: String
    ) {
        let bareEngagementName = app.staticTexts["Engagement name"]
        let payload: [String: Any] = [
            "bareStaticText": clippingElementPayload(bareEngagementName),
            "combinedElement": clippingElementPayload(combinedEngagementName),
            "scenario": scenario
        ]
        guard let data = try? JSONSerialization.data(withJSONObject: payload, options: [.sortedKeys]),
              let text = String(data: data, encoding: .utf8) else {
            print("ACE_MCX19_CLIPPING_ELEMENTS {\"error\":\"serialization-failed\"}")
            return
        }
        print("ACE_MCX19_CLIPPING_ELEMENTS \(text)")
    }

    private func clippingElementPayload(_ element: XCUIElement) -> [String: Any] {
        guard element.exists else { return ["available": false] }
    ...
    private func assertAccessibilityAudit(in app: XCUIApplication, scenario: String) throws {
        recordAccessibilityAuditEvidence(in: app, scenario: scenario, marker: "pre-audit")
        defer {
            recordAccessibilityAuditEvidence(in: app, scenario: scenario, marker: "post-audit")
        }
    ...
    private func recordAccessibilityAuditEvidence(
        in app: XCUIApplication,
        scenario: String,
        marker: String
    ) {
        logAccessibilityAuditMarker(in: app, scenario: scenario, marker: marker)
        addScreenshot(of: app, named: "Accessibility audit — \(scenario) — \(marker)")
    }

    private func logAccessibilityAuditMarker(
        in app: XCUIApplication,
        scenario: String,
        marker: String
    ) {
        let trackedElements = [
            "copyConfirmation": app.staticTexts["Copied Engagement name."],
            "engagementName": app.staticTexts["Engagement name"],
            "noActions": app.staticTexts["No actions are available."],
            "noConclusion": app.staticTexts["No conclusion is available."]
        ]
        var elements: [String: Any] = [:]
        for (name, element) in trackedElements {
            elements[name] = Self.auditEvidenceElementPayload(element)
        }
        let payload: [String: Any] = [
            "appFrame": Self.frameJSON(app.frame),
            "elements": elements,
            "marker": marker,
            "scenario": scenario
        ]
        guard let data = try? JSONSerialization.data(withJSONObject: payload, options: [.sortedKeys]),
              let text = String(data: data, encoding: .utf8) else {
            print("ACE_A11Y_AUDIT_MARKER {\"error\":\"serialization-failed\"}")
            return
        }
        print("ACE_A11Y_AUDIT_MARKER \(text)")
    }

    private static func auditEvidenceElementPayload(_ element: XCUIElement) -> [String: Any] {
        guard element.exists else { return ["available": false] }
        let frame = element.frame
        return [
            "available": true,
            "frame": Self.frameJSON(frame),
            "hittable": element.isHittable,
            "identifier": limitedAuditText(element.identifier),
            "label": limitedAuditText(element.label)
        ]
    }

    private static func accessibilityAuditTypeSymbolsJSON() -> String {
        let payload: [String: Any] = [
            "contrast": NSNumber(value: XCUIAccessibilityAuditType.contrast.rawValue),
            "textClipped": NSNumber(value: XCUIAccessibilityAuditType.textClipped.rawValue)
        ]
        guard let data = try? JSONSerialization.data(withJSONObject: payload, options: [.sortedKeys]),
              let text = String(data: data, encoding: .utf8) else {
            return "{\"contrast\":null,\"textClipped\":null}"
        }
        return text
    }
    ...
    private static func accessibilityIssueJSON(_ issue: XCUIAccessibilityAuditIssue, scenario: String) -> String {
        ...
        let payload: [String: Any] = [
            ...
            "auditTypeSymbols": auditTypeSymbols(issue.auditType),
    ...
    private static func auditTypeSymbols(_ auditType: XCUIAccessibilityAuditType) -> [String] {
        var symbols: [String] = []
        if auditType.contains(.contrast) {
            symbols.append("contrast")
        }
        if auditType.contains(.textClipped) {
            symbols.append("textClipped")
        }
        return symbols
    }
