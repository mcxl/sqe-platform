...
for checkpoint in sorted((run/'batches').glob('*/checkpoint.json')):
 checkpoint_data=json.loads(checkpoint.read_text());assert checkpoint_data['status']=='passed'
 batch=checkpoint.parent;record=checkpoint_data['record'];case_ids={c['id'] for c in record['caseRecords']};batches.append(batch.name)
...
coverage_capture_count=len(images)
settings_phases=[]
for summary in sorted((run/'native-settings').glob('*/summary.json')):
 phase=summary.parent
 if not (phase/'attachments/manifest.json').exists():continue
 manifest=json.loads((phase/'attachments/manifest.json').read_text())
 exported={v['exportedFileName']:v for test in manifest for v in test.get('attachments',[])}
 settings_phases.append(phase.name)
 for p in sorted((phase/'attachments').glob('*.png')):
  raw=p.read_bytes();assert raw[:8]==b'\x89PNG\r\n\x1a\n'
  item=exported.get(p.name);assert item is not None,p
  images.append({'path':str(p),'batch':'settings/'+phase.name,'caseId':None,'nativeName':item.get('suggestedHumanReadableName',''),'bytes':len(raw),'sha256':hashlib.sha256(raw).hexdigest(),'dimensions':list(struct.unpack('>II',raw[16:24]))})
groups={}
for item in images:groups.setdefault(item['sha256'],[]).append(item)
result={'run':str(run),'candidateCommit':json.loads((d/'clean-candidate.json').read_text())['commit'],'indexedUTC':datetime.datetime.now(datetime.timezone.utc).isoformat(),'completedBatches':batches,'nativeCaptureCount':len(images),'coverageCaptureCount':coverage_capture_count,'settingsCaptureCount':len(images)-coverage_capture_count,'settingsPhases':settings_phases,'uniqueImageCount':len(groups),'collectionSeconds':round(time.monotonic()-start,3),'images':images,'uniqueImages':[{'sha256':h,'representative':items[0],'allCapturePaths':[i['path'] for i in items],'caseIds':sorted({i['caseId'] for i in items if i['caseId']})} for h,items in groups.items()]}
