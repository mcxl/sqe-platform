...
assert candidate['commit']==json.loads((d/'review-repair-integration.json').read_text())['candidate']
...
assert json.loads((d/'source-impact-and-reuse.json').read_text())['candidate']==candidate['commit']
...
# Prove the generic build for this exact clean candidate before starting any pilot test.
build_evidence=d/'final-candidate-build';build_evidence.mkdir(exist_ok=False)
try:
 build_record=m.build(candidate,build_evidence)
except Exception as exc:
 record.update(finishedUTC=datetime.datetime.now(datetime.timezone.utc).isoformat(),exit=2,status='build failed; no pilot test started',error=str(exc),candidateUnchanged=m.candidate()==candidate,finalBootStates=states())
 record['bootStatesRestored']=record['initialBootStates']==record['finalBootStates']
 execution.write_text(json.dumps(record,indent=2)+'\n')
 sys.exit(2)
(d/'complete-pilot-build.json').write_text(json.dumps(build_record,indent=2)+'\n')
