...
max_settings_bytes=max(m.directory_bytes(p) for p in (run/'native-settings').iterdir())
max_settings_images=max(len(list((p/'attachments').glob('*.png'))) for p in (run/'native-settings').iterdir())
projected=len(remaining)*(max_batch+max_settings_bytes)+4*max_settings_bytes
free=shutil.disk_usage(root).free
projection={'purpose':'Measurement only; no836-case run authorised by this goal','remainingCases':len(remaining),'remainingBatches':len(remaining_batches),'pilotMaxRawBatchBytes':max_batch,'rawPerRemainingCaseUpperBoundBytes':max_batch+max_settings_bytes,'maxSettingsPhaseBytes':max_settings_bytes,'additionalReadRestorePhaseAllowance':4,'remainingRawEvidenceBytes':projected,'pilotMaxCaseImageCount':max_case_images,'pilotMaxUnmappedBatchImageCount':max_unmapped,'remainingImageCount':len(remaining)*max_case_images+len(remaining_batches)*max_unmapped+(len(remaining)+4)*max_settings_images,'maxSettingsPhaseImages':max_settings_images,'scratchBytes':scratch,'requiredFreeBytes':2*projected+scratch+m.MIN_FREE_BYTES,'observedMacFreeBytes':free,'storageThresholdPassed':free>=2*projected+scratch+m.MIN_FREE_BYTES,'reviewThreshold':'Pending measured image inspection','limitation':'Largest observed coverage batch plus largest Settings phase per remaining case, plus four read/restoration phases. This is a conservative allowance, not measured per-case bundle size'}
record={'status':'passed native integrity; image inspection and archive retrieval remain separate','candidate':candidate,'verifiedUTC':datetime.datetime.now(datetime.timezone.utc).isoformat(),'manifest':str(manifest),'manifestSha256':m.sha256(manifest),'caseCount':22,'auditedCases':20,'layoutOnlyCases':2,'auditInvocations':36,'nativeCoverageTests':6,'nativeSettingsTests':len(settings),'unexpectedSkips':0,'sourceUnchanged':True,'restorationPassed':True,'nativeCoverageSeconds':sum(x['seconds'] for x in batch_commands),'nativeSettingsSeconds':sum(x['seconds'] for x in settings),'buildSecondsBeforePilot':json.loads((Path(pilot['build']['buildLog']).parent/'build-command.json').read_text())['seconds'],'buildCount':1,'nativeCaptureCount':sum(len(list(Path(b['attachmentDirectory']).glob('*.png'))) for b in pilot['batches']),'uniqueImageHashes':sorted(image_hashes),'resourceProjection':projection,'caseRecords':case_rows,'settings':settings,'integrityVerificationSeconds':round(time.monotonic()-started,3)}
