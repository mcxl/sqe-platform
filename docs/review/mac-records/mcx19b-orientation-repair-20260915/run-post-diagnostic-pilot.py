...
candidate=json.loads((stage/'margin-clean-candidate-v3.json').read_text())
assert m.candidate()==candidate and candidate['status']=='clean'
probe=stage/'08-copy-confirmation-diagnosis/native-uninstrumented-prefix-probe'
probe_execution=json.loads((probe/'execution.json').read_text())
probe_result=json.loads((probe/'result.json').read_text())
probe_images=json.loads((probe/'image-inspection.json').read_text())
assert probe_execution['exit']==0 and probe_execution['candidateUnchanged'] and probe_execution['bootStatesRestored']
assert probe_execution['candidate']==candidate and probe_images['status']=='passed'
assert (probe_result['caseCount'],probe_result['auditCount'],probe_result['auditInvocationCount'])==(5,5,9)
paths={role:stage/'margin-readiness-v3'/(role+'-readiness.json') for role in m.READINESS_ROLES}
m.pilot_readiness(candidate,paths)
focus=json.loads((stage/'07-scroll-margin-diagnosis/native-focus-v3/acceptance.json').read_text())
assert focus['status']=='passed' and focus['candidate']==candidate['commit']
assert focus['nativeAuditInvocations']==2 and focus['restoration']=='passed'
review=json.loads((stage/'margin-sol-review-v3.json').read_text())
assert review['candidate']==candidate['commit'] and review['verdict']=='ship for source only'
...
record['purpose']='One complete22-case execution after focused diagnosis; does not resolve or replace historical failures.'
record['historicalFailure']='margin-pilot-runs/20260915T100107Z-pilot-a8d5a906fdf3; Copy action contrast finding remains unresolved.'
record['acceptance']='Pending native execution, images, restoration, evidence review, private archive/retrieval and disposition of unresolved findings.'
record['priorDiagnostic']=str(probe)
(stage/'post-diagnostic-pilot-execution.json').write_text(json.dumps(record,indent=2)+'\n')
args=['--evidence-root',str(stage/'post-diagnostic-pilot-runs'),'pilot','--pocock-evidence',str(paths['pocock']),'--functional-evidence',str(paths['functional']),'--evidence-gate',str(paths['evidence-gate']),'--private-input-record',str(paths['private-input'])]
...
def retain_simctl_commands(args, **kwargs):
    ...
    if len(args)>2 and args[:2]==['xcrun','simctl'] and args[2] in {'boot','shutdown','bootstatus'}:
        with (stage/'post-diagnostic-pilot-simulator-commands.jsonl').open('a') as stream:
