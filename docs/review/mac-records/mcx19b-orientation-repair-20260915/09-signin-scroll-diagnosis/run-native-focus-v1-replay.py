...
evidence=stage/'09-signin-scroll-diagnosis/native-focus-v1-replay'
...
record={'startedUTC':datetime.datetime.now(datetime.timezone.utc).isoformat(),'candidate':candidate,'status':'running','question':'Does the identical Settings tap repeat its missed-input failure? If settings pass, test the pending offscreen Sign in correction.','scope':'single iPhone17 Sign in, dark/maximum-text/landscape, all flags on; focused corrective check only','pilotCredit':0,'appCorrectionAttempt':0,'testHelperCorrectionAttempt':1,'unchangedSettingsReplay':1,'observerEffect':'Only offscreen-control lookup and collection scrolling changed; no app change or pilot credit. Historical Copy contrast cause remains unresolved.','initialBootStates':states(),'scriptSha256':hashlib.sha256(Path(__file__).read_bytes()).hexdigest()}
...
def observed(args,**kwargs):
 ...
 return value
m.command=observed
previous=stage/'09-signin-scroll-diagnosis/native-focus-v1'
previous_execution=json.loads((previous/'execution.json').read_text())
assert previous_execution['candidate']==candidate and previous_execution['candidateUnchanged'] and previous_execution['bootStatesRestored']
built=json.loads((previous/'build-command.json').read_text());assert built['exit']==0
template=m.generated_xctestrun(previous/'build')
products=previous/'build/Build/Products'
build_record={"identity":m.sha256(template),"template":str(template),"buildCommand":built["command"],"buildExit":0,"buildLog":str(previous/'build.log'),"buildLogSha256":m.sha256(previous/'build.log'),"buildResultBundle":str(previous/'build.xcresult'),"buildResultBundleSha256":m.directory_hash(previous/'build.xcresult'),"products":str(products),"productSha256":m.directory_hash(products),"candidate":candidate,"scheme":m.SCHEME}
(evidence/'reused-build.json').write_text(json.dumps(build_record,indent=2)+'\n')
def reuse_build(current,evidence_path,scheme=m.SCHEME):
    assert current==candidate and scheme==m.SCHEME
    assert m.sha256(template)==build_record["identity"] and m.directory_hash(products)==build_record["productSha256"]
    return build_record
m.build=reuse_build
