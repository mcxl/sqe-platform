...
assert candidate['commit']=='c8b10875658e5fbd6a66dea1fbcec9bb4a2f5b0d'
focus=d/'native-screen-focus-v1'
...
assert (f['caseCount'],f['auditCount'],f['auditInvocationCount'])==(1,1,1)
gate=json.loads((d/'native-capture-evidence-gate/gate-integrity.json').read_text())
assert gate['status']=='passed' and gate['candidate']==candidate['commit']
assert json.loads((d/'native-capture-evidence-gate/image-inspection.json').read_text())['status']=='passed'
paths={role:d/'readiness'/(role+'-readiness.json') for role in m.READINESS_ROLES}
m.pilot_readiness(candidate,paths)
review=json.loads((d/'signin-capture-sol-review.json').read_text())
