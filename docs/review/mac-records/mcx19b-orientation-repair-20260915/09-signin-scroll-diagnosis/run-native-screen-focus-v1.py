...
evidence=stage/'09-signin-scroll-diagnosis/native-screen-focus-v1'
...
record={'startedUTC':datetime.datetime.now(datetime.timezone.utc).isoformat(),'candidate':candidate,'status':'running','question':'Does full-screen capture retain the complete landscape Sign in view while the same target checks and unrestricted audit pass?','scope':'single iPhone17 Sign in, dark/maximum-text/landscape, all flags on; focused corrective check only','pilotCredit':0,'appCorrectionAttempt':0,'testHelperCorrectionAttempt':1,'captureCorrectionAttempt':1,'observerEffect':'Only coverage screenshot capture changed from the preceding passing focused case; no app change or pilot credit. Historical Copy contrast cause remains unresolved.','initialBootStates':states(),'scriptSha256':hashlib.sha256(Path(__file__).read_bytes()).hexdigest()}
