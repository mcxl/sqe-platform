...
probe=s/'serial-negative-probe-worktree'
out=s/'05-serial-native-gate'
...
m=load(repo,'serial_positive');n=load(probe,'serial_negative')
...
assert positive==json.loads((s/'serial-clean-candidate.json').read_text())
...
result={'startedUTC':datetime.datetime.now(datetime.timezone.utc).isoformat(),'status':'running','positiveCandidate':positive,'negativeCandidate':negative,'question':'Does the serial runner retain a real failing coverage batch, restore original device settings, then pass the same case with the correct assertion?','scope':'One medium-text portrait/light release layout case on iPhone17. No audits or pilot acceptance credit.','checks':{}}
def save(phase):
    result['phase']=phase
...
try:
    ...
    save('deliberate opposite-orientation assertion; expected native failure')
    caught=None
    ...
    assert len(summary['testFailures'])==1 and 'Native app frame does not match requested landscape orientation' in summary['testFailures'][0]['failureText'],summary
    ...
    save('same release layout case with correct orientation assertion; expected pass')
...
sys.exit(0 if result['status'].startswith('native gate passed') else 2)
