...
evidence=stage/'08-copy-confirmation-diagnosis/native-prefix-probe'
...
record={'startedUTC':datetime.datetime.now(datetime.timezone.utc).isoformat(),'candidate':candidate,'status':'running','question':'Does restoring the original four preceding cases reproduce the Copy contrast failure with the same clean frame-logging candidate?','scope':'original five-case iPhone17 light/large/portrait prefix, all flags off; diagnostic only','pilotCredit':0,'appCorrectionAttempt':0,'diagnosticProbe':2,'observerEffect':'Extra frame queries change timing; a pass does not prove a correction','initialBootStates':states(),'scriptSha256':hashlib.sha256(Path(__file__).read_bytes()).hexdigest()}
...
def observed(args,**kwargs):
 ...
 return value
m.command=observed
cases=tuple(c for c in m.pilot_cases() if c.device=='iPhone 17' and c.settings.appearance=='light')
assert len(cases)==5
