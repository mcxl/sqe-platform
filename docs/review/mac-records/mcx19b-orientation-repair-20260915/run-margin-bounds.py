...
evidence=stage/'07-scroll-margin-diagnosis/native-focus-v3'
...
record={'startedUTC':datetime.datetime.now(datetime.timezone.utc).isoformat(),'candidate':candidate,'status':'running','question':'Does clipping the release viewport prevent status-bar overlap while both unrestricted native noConclusion audits pass?','scope':'one noConclusion case, iPhone17, light,large,portrait,all flags off','pilotCredit':0,'correctiveAttempt':2,'initialBootStates':states(),'scriptSha256':hashlib.sha256(Path(__file__).read_bytes()).hexdigest()}
