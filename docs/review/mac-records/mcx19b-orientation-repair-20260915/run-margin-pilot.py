...
(stage/'margin-pilot-execution.json').write_text(json.dumps(record,indent=2)+'\n')
args=['--evidence-root',str(stage/'margin-pilot-runs'),'pilot','--pocock-evidence',str(paths['pocock']),'--functional-evidence',str(paths['functional']),'--evidence-gate',str(paths['evidence-gate']),'--private-input-record',str(paths['private-input'])]
...
def retain_simctl_commands(args, **kwargs):
    ...
    if len(args)>2 and args[:2]==['xcrun','simctl'] and args[2] in {'boot','shutdown','bootstatus'}:
        with (stage/'margin-pilot-simulator-commands.jsonl').open('a') as stream:
