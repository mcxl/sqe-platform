...
run=stage/'post-diagnostic-pilot-runs/20260915T110638Z-pilot-71f4f52cf8a8'
...
execution=json.loads((stage/'post-diagnostic-pilot-execution.json').read_text())
...
for p in sorted((run/'restoration').glob('*readback.json')):
 d=json.loads(p.read_text());assert d['expected']==d['observed']
...
record={'status':'passed native integrity; image inspection and archive retrieval remain separate','candidate':candidate,'verifiedUTC':datetime.datetime.now(datetime.timezone.utc).isoformat(),'manifest':str(manifest),'manifestSha256':m.sha256(manifest),'caseCount':22,'auditedCases':20,'layoutOnlyCases':2,'auditInvocations':36,'nativeCoverageTests':6,'nativeSettingsTests':len(settings),'unexpectedSkips':0,'sourceUnchanged':True,'restorationPassed':True,'nativeCoverageSeconds':sum(x['seconds'] for x in batch_commands),'nativeSettingsSeconds':sum(x['seconds'] for x in settings),'buildSeconds':json.loads((run/'build-command.json').read_text())['seconds'],'nativeCaptureCount':sum(len(list(Path(b['attachmentDirectory']).glob('*.png'))) for b in pilot['batches']),'uniqueImageHashes':sorted(image_hashes),'resourceProjection':projection,'caseRecords':case_rows,'settings':settings,'integrityVerificationSeconds':round(time.monotonic()-started,3)}
with (stage/'post-diagnostic-pilot-native-integrity.json').open('x') as stream:json.dump(record,stream,indent=2)
