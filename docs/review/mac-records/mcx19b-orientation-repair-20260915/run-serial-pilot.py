...
spec=importlib.util.spec_from_file_location('serial_pilot',root/'tools/ace_ios_local.py')
m=importlib.util.module_from_spec(spec);sys.modules[spec.name]=m;spec.loader.exec_module(m)
candidate=json.loads((stage/'serial-clean-candidate.json').read_text())
assert m.candidate()==candidate and candidate['status']=='clean'
paths={role:stage/'serial-readiness'/(role+'-readiness.json') for role in m.READINESS_ROLES}
m.pilot_readiness(candidate,paths)
gate=json.loads((stage/'05-serial-native-gate/result.json').read_text())
assert gate['status']=='native gate passed; image inspection pending'
inspection=json.loads((stage/'05-serial-native-gate/image-inspection.json').read_text())
...
(stage/'serial-pilot-execution.json').write_text(json.dumps(record,indent=2)+'\n')
args=['--evidence-root',str(stage/'serial-pilot-runs'),'pilot','--pocock-evidence',str(paths['pocock']),'--functional-evidence',str(paths['functional']),'--evidence-gate',str(paths['evidence-gate']),'--private-input-record',str(paths['private-input'])]
code=m.main(args)
