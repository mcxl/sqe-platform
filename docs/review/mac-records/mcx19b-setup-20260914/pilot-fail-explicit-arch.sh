...
cd "$repo" || exit 1
git diff --binary > "$evidence/pilot-fail-explicit-arch-source.patch"
shasum -a 256 ios/ACEClientApp/ACEClientAppTests/ACEClientAppTests.swift > "$evidence/pilot-fail-explicit-arch-source.sha256"
date -u '+%Y-%m-%dT%H:%M:%SZ' > "$evidence/pilot-fail-explicit-arch.started"
caffeinate -i xcodebuild test \
  ...
  -resultBundlePath "$evidence/pilot-fail-explicit-arch.xcresult" \
  ...
  > "$evidence/pilot-fail-explicit-arch.log" 2>&1
result=$?
printf '%s\n' "$result" > "$evidence/pilot-fail-explicit-arch.exit"
date -u '+%Y-%m-%dT%H:%M:%SZ' > "$evidence/pilot-fail-explicit-arch.finished"
tail -n 25 "$evidence/pilot-fail-explicit-arch.log"
exit "$result"
