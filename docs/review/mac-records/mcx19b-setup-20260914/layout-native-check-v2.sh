...
case "$selector" in testFictionalReleaseHasApprovedCopyControls|testClippingNoConclusionStandaloneAudit|testClippingNoActionsStandaloneAudit) ;; *) exit 2;; esac
...
/Library/Frameworks/Python.framework/Versions/3.14/bin/python3 - "$products" "$tag" "$appearance" <<'PY'
...
targets[0].setdefault('EnvironmentVariables', {})['ACE_UI_TEST_APPEARANCE'] = appearance
targets[0]['EnvironmentVariables']['ACE_UI_TEST_RETAIN_INITIAL_AUDIT_SCREENSHOT'] = '1'
