...
git rev-parse HEAD > "$run_dir/ui-build-v2.commit"
git diff --binary > "$run_dir/ui-build-v2-source.patch"
shasum -a 256 ios/ACEClientApp/ACEClientApp/*.swift ios/ACEClientApp/ACEClientAppUITests/ACEClientAppUITests.swift > "$run_dir/ui-build-v2-source.sha256"
date -u +%FT%TZ > "$run_dir/ui-build-v2.started"
caffeinate -i xcodebuild build-for-testing \
  ...
  -resultBundlePath "$run_dir/ui-build-v2.xcresult" \
  ...
  > "$run_dir/ui-build-v2.log" 2>&1
result=$?
printf '%s\n' "$result" > "$run_dir/ui-build-v2.exit"
date -u +%FT%TZ > "$run_dir/ui-build-v2.finished"
tail -n 30 "$run_dir/ui-build-v2.log"
exit "$result"
