...
cd "$repo" || exit 1
git diff --binary > "$evidence/pilot-fail-simulator-source.patch"
shasum -a 256 ios/ACEClientApp/ACEClientAppTests/ACEClientAppTests.swift > "$evidence/pilot-fail-simulator-source.sha256"
date -u '+%Y-%m-%dT%H:%M:%SZ' > "$evidence/pilot-fail-simulator.started"
caffeinate -i xcodebuild test \
  ...
  -resultBundlePath "$evidence/pilot-fail-simulator.xcresult" \
  ...
  > "$evidence/pilot-fail-simulator.log" 2>&1
result=$?
printf '%s\n' "$result" > "$evidence/pilot-fail-simulator.exit"
date -u '+%Y-%m-%dT%H:%M:%SZ' > "$evidence/pilot-fail-simulator.finished"
tail -n 25 "$evidence/pilot-fail-simulator.log"
exit "$result"
