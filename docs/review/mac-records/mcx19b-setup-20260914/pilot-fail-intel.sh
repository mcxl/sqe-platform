...
cd "$repo" || exit 1
git diff --binary > "$evidence/pilot-fail-intel-source.patch"
shasum -a 256 ios/ACEClientApp/ACEClientAppTests/ACEClientAppTests.swift > "$evidence/pilot-fail-intel-source.sha256"
date -u '+%Y-%m-%dT%H:%M:%SZ' > "$evidence/pilot-fail-intel.started"
caffeinate -i xcodebuild test \
  ...
  -resultBundlePath "$evidence/pilot-fail-intel.xcresult" \
  ...
  > "$evidence/pilot-fail-intel.log" 2>&1
result=$?
printf '%s\n' "$result" > "$evidence/pilot-fail-intel.exit"
date -u '+%Y-%m-%dT%H:%M:%SZ' > "$evidence/pilot-fail-intel.finished"
tail -n 25 "$evidence/pilot-fail-intel.log"
exit "$result"
