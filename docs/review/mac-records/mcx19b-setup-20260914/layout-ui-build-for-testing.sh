...
git rev-parse HEAD > "$run_dir/ui-build.commit"
git diff --binary > "$run_dir/ui-build-source.patch"
shasum -a 256 ios/ACEClientApp/ACEClientApp/*.swift ios/ACEClientApp/ACEClientAppUITests/ACEClientAppUITests.swift > "$run_dir/ui-build-source.sha256"
date -u +%FT%TZ > "$run_dir/ui-build.started"
caffeinate -i xcodebuild build-for-testing \
  ...
  -resultBundlePath "$run_dir/ui-build.xcresult" \
...
result=$?
printf '%s\n' "$result" > "$run_dir/ui-build.exit"
date -u +%FT%TZ > "$run_dir/ui-build.finished"
tail -n 30 "$run_dir/ui-build.log"
exit "$result"
