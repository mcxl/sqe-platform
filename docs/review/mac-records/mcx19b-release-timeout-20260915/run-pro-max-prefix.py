...
device,settings,allcases=next(group for group in m.coverage_batches(m.pilot_cases()) if group[0]=='iPhone 17 Pro Max' and group[1].appearance=='light' and group[1].contentSize=='large')
cases=allcases[:2]
...
result={'startedUTC':datetime.datetime.now(datetime.timezone.utc).isoformat(),'question':'Do the same signIn and release checks pass on the other required simulator? This verifies device coverage; it does not isolate a cause because device and active simulator set differ.','candidate':candidate,'casePayloads':[c.payload() for c in cases],'originalBootState':initial,'retainedBuild':build,'correctionAttempt':False,'restorationFailures':[]}
