...
For implementation, refactoring, debugging, pipeline, template, and test changes,
use `$astra-advisor:orchestration` automatically unless the user selects another workflow.
Keep Astra as the primary advisor at the user-selected reasoning effort.
Do not invoke Sol Advisor or require Sol / High for the primary session.
Continue in the current Astra task when its scope and context are suitable.

- Use native Terra subagents for implementation and obtain a fresh Sol reviewer.
- These SQE implementation and review requirements constrain Astra's model selection.
- These document rules, specialist SQE workflow, human decisions, and final review
  requirements remain authoritative; Astra Advisor does not replace professional review.
